...
public class GameUI extends javax.swing.JFrame {
    ...
    Action LeftAction;
    Action RightAction;
    int CoordsX = 1; //player tile location
    int CoordsY = 1;
    int length = 30; //map size
    int width = 70;
    ...
    PlayerStats player = new PlayerStats(playerName, 20, 2, 4, map, 1, 1, length, width);
     ...
     */
    public GameUI() { 
        in.fillItems(); //generating inventory
        events.mapReplacer(); //reset map
        initComponents();
        //Positions
    ...
    public String getPlayerName() { //Player selects their name
        NamingScreen dialog = new NamingScreen(null, true); //opens inventory window
        dialog.setLocationRelativeTo(null);
        dialog.setVisible(true);
        return dialog.getName();
    }
    
    public void updateUI() { //refresh the UI when a change happens 
        playerNameLabel.setText(player.getName()); //setting top UI values
        ...
        inventoryTextBox.setText(""); //filling inventory with owned items
        for (int i = 0; i < in.getInventoryLength(); i++) {
            String n = in.getItems(i);
            if (n.equals("")){ //if the inventory space if empty, it will not print
                
            }
            else {
                inventoryTextBox.append(n+"\n");
            }
        }
    }

    public void gameWin() { //for when the player wins the game
        WinScreen dialog = new WinScreen(null, true); //Gets if player wants to start new game or quit
        dialog.setLocationRelativeTo(null);
        dialog.setVisible(true);
        ...
        else if(selection == 2) { //reset map and player location
            CoordsX = 1;
            CoordsY = 1;
            events.mapReplacer();
            for (int x = 0; x < 70; x++) {
    ...
    private void initComponents() {
                ...
                .addGroup(BackgroundLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(BackgroundLayout.createSequentialGroup()
                        .addGroup(BackgroundLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            .addComponent(playerHealthLabel, javax.swing.GroupLayout.DEFAULT_SIZE, 100, Short.MAX_VALUE)
        ...
        BackgroundLayout.linkSize(javax.swing.SwingConstants.HORIZONTAL, new java.awt.Component[] {attackButton, inventoryButton, jButton1, jButton4});
        ...
        BackgroundLayout.linkSize(javax.swing.SwingConstants.VERTICAL, new java.awt.Component[] {attackButton, inventoryButton, jButton1, jButton4});
    ...
    private void attackButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_attackButtonMouseClicked
        ...
        if (combat == true) {
            ...
            player.statusCheck(events, eventTextBox);
            updateUI();
            if(player.getCurrentHealth() <= 0) { //checks if player is out of hp, ends game (game over)
                GameOver end = new GameOver(null, true, player.getName(), player.getLevel(), bad1.getName()); //opens inventory window
                end.setLocationRelativeTo(null);
                end.setVisible(true);
                System.exit(0);
            }
            if (bad1.getCurrentHealth() <= 0) { //check if enemy is out of hp, ends combat
                player.clearStatus();
            ...
            bad1.statusCheck(events, eventTextBox);
            bad1.actionDo(player, events, eventTextBox);
            updateUI();
        }
        else if (combat == false) { //if teh player attacks while not in combat (nothing happends)
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        // TODO add your handling code here:
        in.equip(player, eventTextBox, true); //note: delete mode is on
        updateUI();
    }//GEN-LAST:event_jButton4ActionPerformed

    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        // TODO add your handling code here:
        TutorialScreen dialog = new TutorialScreen(null, true); //opens tutorial window
        dialog.setLocationRelativeTo(null);
        dialog.setVisible(true);
